...
class FusionGate(nn.Module):
    ...
    def forward(self, ts_encoding: Tensor, net_encoding: Tensor) -> Tensor:
        """
...
class BrainMamba(nn.Module):
    ...
    def __init__(
        ...
        # Fusion Gate
        self.fusion_gate = FusionGate(d_model=d_model, dropout=dropout)

        # Mutual Information Loss
        self.mi_loss = MutualInformationLoss(d_model=d_model)
    ...
    def forward(
        self,
        timeseries: Tensor,
        adj_matrix: Optional[Tensor] = None,
        functional_systems: Optional[Tensor] = None,
        return_mi_loss: bool = False,
        ...
        # torch.cuda.amp.autocast is deprecated in favor of torch.amp.autocast
        # We need to handle device type
        device_type = "cuda" if torch.cuda.is_available() else "cpu"

        with torch.amp.autocast(device_type=device_type, enabled=self.use_mixed_precision):
            ...
            if functional_systems is None:
                ...
                functional_systems = functional_systems.to(timeseries.device)
        ...
        if return_mi_loss and mi_loss is not None:
    ...
    def inference(self, timeseries: Tensor) -> Tuple[Tensor, Tensor]:
        """
